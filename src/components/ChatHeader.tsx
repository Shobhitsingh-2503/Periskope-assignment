...
const ChatHeader = () => {
  ...
  const maxVisible = 5;
  const visibleAvatars = staticAvatars.slice(0, maxVisible);
  const remaining = staticAvatars.length - maxVisible;
    ...
    <div className="p-0.5 bg-[#ffffff] border-b-1 border-[#F1F0E9] flex items-center flex-row justify-between">
      <div className="flex flex-row items-center">
        <div className="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center text-white mr-3 overflow-hidden">
          {currentChat ? currentChat.name?.charAt(0).toUpperCase() : "?"}
        </div>
        <div className="flex flex-col flex-1">
          ...
          <div className="text-xs text-gray-500 flex items-center">
            ...
            {(!currentChat || currentChat?.participants?.length === 0) &&
              "No participants"}
          </div>
        </div>
      </div>
